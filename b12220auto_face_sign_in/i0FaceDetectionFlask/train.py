...
# 初始化，即训练文件中已有人脸信息
def init():
    names = [x for x in os.listdir("trained_images")]  # 获取该文件夹下的目录名
    print(names)
    if ".DS_Store" in names:
        names.remove(".DS_Store")
    for i in names:
        name = [x for x in os.listdir("./trained_images/" + i)]
        # print(name)
        for j in name:  # 获取每一个文件夹下的图片名称
            add_data(i, "./trained_images/" + i + "/" + j)  # 将文件夹名称及其目录下所有图片名称
...
def recognize_from_upload(file):
    ...
    for k in unknown_encoding:  # 遍历多张脸
        ...
        for i in face_datas:  # 遍历已训练的人脸编码信息
            if face_recognition.compare_faces(face_datas[i], k, tolerance=0.3)[
                0
            ]:  # 比较字典中与待识别图片,第一个参数为数组！！！
                img_info.append(i)
                find = True
                break
        if not find:
