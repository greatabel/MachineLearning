...
parser.add_argument(
    ...
    help="datasets to train on: [train/traditional],[train/cnn],[train/mix],[val/traditional],[val/cnn],[val/color],[val/deblur],[val/frameinterp],[val/superres]",
)
parser.add_argument(
    "--model",
    type=str,
...
parser.add_argument(
    "--nThreads", type=int, default=4, help="number of threads to use in data loader"
)
parser.add_argument(
    "--nepoch", type=int, default=5, help="# epochs at base learning rate"
)
parser.add_argument(
    "--nepoch_decay",
    type=int,
    default=5,
    help="# additional epochs at linearly learning rate",
)
parser.add_argument(
    "--display_freq",
    type=int,
    default=5000,
    help="frequency (in instances) of showing training results on screen",
)
parser.add_argument(
    "--print_freq",
    type=int,
    default=5000,
    help="frequency (in instances) of showing training results on console",
)
parser.add_argument(
    "--save_latest_freq",
    type=int,
    default=20000,
    help="frequency (in instances) of saving the latest results",
)
parser.add_argument(
    "--save_epoch_freq",
    type=int,
...
parser.add_argument(
    "--display_port", type=int, default=8001, help="visdom display port"
)
parser.add_argument("--use_html", action="store_true", help="save off html pages")
parser.add_argument(
    "--checkpoints_dir", type=str, default="checkpoints", help="checkpoints directory"
)
parser.add_argument(
    "--name", type=str, default="tmp", help="directory name for training"
)
...
for epoch in range(1, opt.nepoch + opt.nepoch_decay + 1):
    ...
    for i, data in enumerate(dataset):
        ...
        if total_steps % opt.print_freq == 0:
            ...
            t2o = (time.time() - epoch_start_time) / 3600.0
            t2 = t2o * D / (i + 0.0001)
            ...
            for key in errors.keys():
                visualizer.plot_current_errors_save(
                    epoch,
