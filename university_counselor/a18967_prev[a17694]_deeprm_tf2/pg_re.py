...
def process_all_info(trajs):
    ...
    for traj in trajs:
        enter_time.append(
            np.array(
                [
                    traj["info"].record[i].enter_time
                    for i in range(len(traj["info"].record))
                ]
            )
        )
        finish_time.append(
            np.array(
                [
                    traj["info"].record[i].finish_time
                    for i in range(len(traj["info"].record))
                ]
            )
        )
        job_len.append(
            np.array(
                [traj["info"].record[i].len for i in range(len(traj["info"].record))]
            )
        )
...
def plot_lr_curve(
    ...
    for k in ref_discount_rews:
        ax.plot(
            np.tile(
                np.average(np.concatenate(ref_slow_down[k])), len(slow_down_lr_curve)
            ),
            linewidth=2,
            label=k,
        )
...
def get_traj_worker(rl, env, pa):
    ...
    all_slowdown = (finish_time[finished_idx] - enter_time[finished_idx]) / job_len[
        finished_idx
    ]
...
def launch(pa, pg_resume=None, render=False, repre="image", end="no_new_job"):
    ...
    rl = Policy_Network.PolicyGradient(
