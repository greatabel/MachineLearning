...
class Env:
        ...
        if nw_len_seqs is None or nw_size_seqs is None:
            ...
            for i in range(pa.num_res):
                ...
                print(
                    "Load on # "
                    + str(i)
                    + " resource dimension is "
                    + str(self.workload[i])
                )
            self.nw_len_seqs = np.reshape(
                self.nw_len_seqs, [self.pa.num_ex, self.pa.simu_len]
            )
            self.nw_size_seqs = np.reshape(
                self.nw_size_seqs, [self.pa.num_ex, self.pa.simu_len, self.pa.num_res]
            )
        else:
    ...
    def get_new_job_from_seq(self, seq_no, seq_idx):
        new_job = Job(
            res_vec=self.nw_size_seqs[seq_no, seq_idx, :],
    ...
    def observe(self):
        if self.repre == "image":
            ...
            for i in range(self.pa.num_res):
                ...
                image_repr[:, ir_pt : ir_pt + self.pa.res_slot] = self.machine.canvas[
                    i, :, :
                ]
                ...
                for j in range(self.pa.num_nw):
                    ...
                    if self.job_slot.slot[j] is not None:  # fill in a block of work
                        image_repr[
                            : self.job_slot.slot[j].len,
                            ir_pt : ir_pt + self.job_slot.slot[j].res_vec[i],
                        ] = 1

                    ir_pt += self.pa.max_job_size

            image_repr[
                : int(self.job_backlog.curr_size / backlog_width),
                ir_pt : ir_pt + backlog_width,
            ] = 1
            if self.job_backlog.curr_size % backlog_width > 0:
                image_repr[
                    int(self.job_backlog.curr_size / backlog_width),
                    ir_pt : ir_pt + self.job_backlog.curr_size % backlog_width,
                ] = 1
            ir_pt += backlog_width

            image_repr[
                :, ir_pt : ir_pt + 1
            ] = self.extra_info.time_since_last_new_job / float(
    ...
    def plot_state(self):
        ...
        for i in range(self.pa.num_res):
            ...
            plt.subplot(
                ...
                1
                + self.pa.num_nw
                + 1,  # first +1 for current work, last +1 for backlog queue
                i * (self.pa.num_nw + 1) + skip_row + 1,
            ...
            for j in range(self.pa.num_nw):
                ...
                if self.job_slot.slot[j] is not None:  # fill in a block of work
                    job_slot[
                        : self.job_slot.slot[j].len, : self.job_slot.slot[j].res_vec[i]
                    ] = 1
        ...
        backlog[
            self.job_backlog.curr_size / backlog_width,
            : self.job_backlog.curr_size % backlog_width,
        ] = 1
        ...
        extra_info = (
            np.ones((self.pa.time_horizon, 1))
    ...
    def step(self, a, repeat=False):
        ...
        if status == "MoveOn":
            ...
            if self.end == "no_new_job":  # end of new job sequence
                ...
                if (
                    self.seq_idx >= self.pa.simu_len
                    and len(self.machine.running_job) == 0
                    and all(s is None for s in self.job_slot.slot)
                    and all(s is None for s in self.job_backlog.backlog)
                ):
                    done = True
                elif (
                    self.curr_time > self.pa.episode_max_length
            ...
            if not done:
                ...
                if (
                    self.seq_idx < self.pa.simu_len
                ):  # otherwise, end of new job sequence, i.e. no new jobs
            ...
            if self.job_backlog.curr_size > 0:
                self.job_slot.slot[a] = self.job_backlog.backlog[
                    0
